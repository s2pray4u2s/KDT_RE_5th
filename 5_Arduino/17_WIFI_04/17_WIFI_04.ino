...
// ---------------- ESP-01 설정 ----------------
SoftwareSerial myESP(2, 3); // D2 = RX, D3 = TX
...
// ---------------- loop ----------------
void loop() {
  delay(2000); // 센서 안정성 및 ESP 버퍼 보호

  float h = myDHT.readHumidity();
  float t = myDHT.readTemperature(); // 섭씨
  ...
  Serial.print(" °C, 습도: "); Serial.println(h);

  sendDataToServer((int)t, (int)h);

  delay(3000); // 서버 과부하 방지
}
